...
def plot_obs_v_sim3(subdir=".",post_iter=None):
    """plot the results for daily, monthly batch and monthly dsi
    ...
    cw_cols = c_oe.columns.map(lambda x: "mass" in x)
    c_oe.loc[:, cw_cols] = c_oe.loc[:, cw_cols].apply(np.log10)
    pname = os.path.join('.', "obs_v_sim_prior.pdf")
    if post_iter is not None:
        pname = os.path.join('.', "obs_v_sim_posterior_{0}.pdf".format(post_iter))

    m = 1
    pp = PdfPages(pname)
    for ireal in range(20):
        s_b_m_d = os.path.join('.', "monthly_model_files_master_{0}".format(ireal))
        s_d_m_d = os.path.join('.', "monthly_model_files_master_{0}_dsi".format(ireal))
        if not os.path.exists(s_d_m_d) or not os.path.exists(s_b_m_d):
            break
        try:
            s_b_pst = pyemu.Pst(os.path.join(s_b_m_d, "freyberg.pst"))
            s_b_oe_pr = pd.read_csv(os.path.join(s_b_m_d, "freyberg.0.obs.csv"), index_col=0)
            ...
            s_d_pst = pyemu.Pst(os.path.join(s_d_m_d, "freyberg.pst"))
            s_d_oe_pr = pd.read_csv(os.path.join(s_d_m_d, "freyberg.0.obs.csv"), index_col=0)
            sw_cols = s_d_oe_pr.columns.map(lambda x: "mass" in x)
            s_d_oe_pr.loc[:, sw_cols] = s_d_oe_pr.loc[:, sw_cols].apply(np.log10)
            dpost_iter = s_d_pst.control_data.noptmax
            if post_iter is not None:
                dpost_iter = post_iter
            s_d_oe_pt = pd.read_csv(os.path.join(s_d_m_d, "freyberg.{0}.obs.csv".format(dpost_iter)),
                                    index_col=0)
            s_d_oe_pt.loc[:, sw_cols] = s_d_oe_pt.loc[:, sw_cols].apply(np.log10)
        ...
        for ogname in ognames:
            ...
            cgobs = cobs.loc[cobs.obsnme.str.contains(ogname), :].copy()
            sgobs = s_b_pst.observation_data.loc[s_b_pst.observation_data.obsnme.str.contains(k0ogname), :].copy()
            dgobs = s_d_pst.observation_data.loc[s_d_pst.observation_data.obsnme.str.contains(k0ogname), :].copy()
            if cgobs.shape[0] == 0:
                raise Exception("complex empty " + ogname)
            if sgobs.shape[0] == 0:
                raise Exception("batch empty " + k0ogname + "," + ogname)
            if dgobs.shape[0] == 0:
                raise Exception("dsi empty " + k0ogname + "," + ogname)

            sgobs.loc[:, "time"] = sgobs.time.apply(float)
            dgobs.loc[:, "time"] = dgobs.time.apply(float)
            cgobs.loc[:, "time"] = cgobs.time.apply(float)
            sgnzobs = sgobs.loc[sgobs.weight > 0, :].copy()

            sgobs.sort_values(by="time", inplace=True)
            dgobs.sort_values(by="time", inplace=True)
            cgobs.sort_values(by="time", inplace=True)
            ...
            ax = axes[1]
            ax.set_title(
                "Figure S{0}B) DSI formulation {1}, replicate {2}, coarse scenario".format(m, label_dict[ogname],
                                                                                             c_oe.index[ireal]),
                loc="left")
            ax.set_xlabel("Simulation Time (days)")
            ax.set_ylabel("{0}".format(units_dict[ogname]))
